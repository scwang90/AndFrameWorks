...
public abstract class AfUpdateService {
	...
	public void checkUpdate(boolean feedback) {
		AfDaemonThread.postTask(new AfDataTask<>(feedback, new CheckUpdateTask()));
	}

	public AfUpdateService(Context context) {
		mContext = context;
		mVersion = AfApplication.getVersion();
	}
	...
	private void start(String url, String version) {
		if (mEntity != null && mEntity.Name.startsWith(version)) {
			...
			entity.DownloadPath = AfApplication.getApp().getWorkspacePath("update");
			entity.Notify = new AfDownloader.NotifyEntity();
			entity.Notify.ContentTitle = AfApplication.getApp().getAppName() + "更新";
			...
			dialog.doShowDialog("可用更新", message,"暂不更新",null,
					"下载更新", new OnClickListener() {
						...
						public void onClick(DialogInterface dialog, int which) {
							startDownLoadUpate(mVersionInfo.downloadUrl, mVersionInfo.serviceVersion);
						}
					});
		}
	}
