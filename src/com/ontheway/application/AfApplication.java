...
import com.ontheway.BuildConfig;
import com.ontheway.activity.AfActivity;
import com.ontheway.activity.AfMainActivity;
import com.ontheway.broadcast.ConnectionChangeReceiver;
import com.ontheway.caches.AfImageCaches;
import com.ontheway.caches.AfSharedPreference;
import com.ontheway.constant.AfFixedPositionEnum;
import com.ontheway.fragment.AfFragment;
import com.ontheway.network.AfFileService;
import com.ontheway.network.AfImageService;
import com.ontheway.network.AfSoapService;
import com.ontheway.network.AfSoapServiceOld;
import com.ontheway.thread.AfDispatch;
import com.ontheway.thread.AfTask;
import com.ontheway.thread.AfThreadWorker;
import com.ontheway.util.AfDeviceInfo;
import com.ontheway.util.AfMD5;
import com.ontheway.util.AfNetwork;
import com.ontheway.util.AfVersion;
import com.ontheway.util.DatabaseUtil;
...
public abstract class AfApplication extends Application {
	...
	public static synchronized AfTask postTaskDelayed(AfTask task, long delay) {
		if (mApp.mWorker != null) {
	...
	// 当前定位状态 默认为 未定位
	protected int mFixedPositionStatus = AfFixedPositionEnum.NONE;
	// 当前主页面
	...
	public synchronized void setNetworkStatus(Object power, int networkState) {
		...
		if (power instanceof ConnectionChangeReceiver) {
			...
			if (mNetworkStatus != AfNetwork.TYPE_NONE) {
				// 如果还没有成功定位
				if (mFixedPositionStatus != AfFixedPositionEnum.FIXED) {
					// 初始化定位信息
				}
			}
		}
	...
	protected void onRestoreInstanceState(AfSharedPreference state) {
		...
		mFixedPositionStatus = state.getInt(STATE_FIXEDPOSITION,
				mFixedPositionStatus);
	...
	protected void onSaveInstanceState(AfSharedPreference state) {
		...
		editor.putInt(STATE_FIXEDPOSITION, mFixedPositionStatus);
