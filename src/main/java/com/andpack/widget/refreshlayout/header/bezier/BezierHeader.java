package com.andpack.widget.refreshlayout.header.bezier;
...
public class BezierHeader extends FrameLayout implements RefreshHeader {
            ...
            for (String color : colors) {
                if (color.matches("[0-9a-fA-F]{6,8}")) {
                    if (color.equals(colors[colors.length - 1])) {
                        setFrontColor(Color.parseColor("#"+color));
                    } else {
                        setBackColor(Color.parseColor("#"+color));
                    }
                }
            }
        }
    ...
    public void setBackColorId(int colorId) {
        setBackColor(ContextCompat.getColor(getContext(), colorId));
    }

    public void setFrontColorId(int colorId) {
        setFrontColor(ContextCompat.getColor(getContext(), colorId));
    }
