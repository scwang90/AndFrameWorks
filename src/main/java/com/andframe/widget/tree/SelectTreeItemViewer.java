...
public abstract class SelectTreeItemViewer<T> extends SelectListItemViewer<T> {
    ...
    protected TreeNode<T> mNode = null;
    protected View mTreeViewContent = null;
    protected LinearLayout mTreeViewLayout = null;
    protected TreeViewItemAdapter<T> mTreeViewAdapter = null;

    public SelectTreeItemViewer() {
    }

    public SelectTreeItemViewer(int layoutId) {
        super(layoutId);
    }
    ...
    public View inflateLayout(View view, TreeViewItemAdapter<T> adapter) {
        //设置适配器
