...
public class TableRow extends LinearLayout implements OnClickListener {
	...
	public TableRow(Context context, Table table) {
		...
		for (int i = 0; i < mViews.length; i++) {
			if (columns[i].Type == Column.CHECKBOX) {
				...
				textcell.setTextSize(TYPE_VALUE, table.TextsizeCell);
				mViews[i] = textcell;
			}
			if ((i & 1) == 1) {
	...
	public void Binding(Table table, List<?> list, int position)
			throws Exception {
		if ((position & 1) == 1) {
		...
		for (int i = 0; i < column.length; i++) {
			if(column[i].Binding.length() > 0){
				...
				if (column[i].Type == Column.CHECKBOX) {
	...
	private void BindingCheck(Column column, LinearLayout layout,
							  Object value) {
	...
	private void BindingText(Column column, TextView textview, Object value) {
		if (value == null) {
			textview.setText("");
		} else {
	...
	// / </summary>
	private Object InvokeMember(Column[] columns, int col, Class<?> type, String[] path,
								Object obj, int index) throws Exception {
		Column column = columns[col];
		Field field = type.getField(path[index]);
		Object value = field.get(obj);
		if (path.length == index + 1) {
			if (column.Type == Column.CHECKBOX) {
				mObject = obj;
				mFiled[col] = field;
