...
public class AfPullDownLayout extends LinearLayout {
	...
	private ReadyForPullDownable mPullDownable = () -> true;
	...
	public void setPullDownable(final ScrollView scrollview){
		if (scrollview != null) {
			this.mPullDownable = () -> scrollview.getScrollY() <= 0;
		}
	}
	...
	public void setMaxScorllHeightDp(int height) {
		...
		this.H = (int) (density * height + 0.5f);
	}
	// ===========================================================
		...
		case MotionEvent.ACTION_UP: {
			if (isBeingDragged) {
				...
				if (onOnPullDownListener != null) {
					int x = Math.round(Math.max(lastMotionY - initMotionY, 0));
					int y = (int)(H-H*H/(x+H));
	...
	protected boolean isReadyForPullDown() {
		return this.mPullDownable == null || mPullDownable.isReadyForPullDown();
	}
	...
	private final class SmoothRunnable implements Runnable {
		...
		public void run() {
			...
			if (startTime == -1) {
				...
				if (!smoothable.onSmooth(value, valuefrom, valueto)) {
					running = false;
