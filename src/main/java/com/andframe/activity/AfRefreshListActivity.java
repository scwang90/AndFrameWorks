...
//        onLoad();
     ...
     * @return 数据页面切换器
     */
    protected abstract AfFrameSelector newAfFrameSelector(Pager pager);
     ...
     * @return 加载页面模块
     */
    protected abstract AfModuleProgress newModuleProgress(Pager pager);
     ...
     * @return 空数据页面模块
     */
    protected abstract AfModuleNodata newModuleNodata(Pager pager);
    ...
    public boolean onRefresh() {
        return postTask(new AbListViewTask(AfListViewTask.TASK_REFRESH)).setListener(mListView).prepare();
    }
     ...
     */
    @Override
    protected final void onTaskLoaded(@SuppressWarnings("UnusedParameters") AfHandlerTask task, List<T> list) {
        super.onTaskLoaded(task, list);
    }
    ...
    protected boolean onLoaded(AbListViewTask task, boolean isfinish, List<T> ltdata, Date cachetime) {
        ...
        if (isfinish && ltdata != null && ltdata.size() > 0) {
            //设置上次刷新缓存时间
    ...
    protected boolean onRefreshed(AbListViewTask task, boolean isfinish, List<T> ltdata) {
        if (isfinish) {
            ...
            if (ltdata != null && ltdata.size() > 0) {
                ...
                mAdapter.set(ltdata == null ? new ArrayList<>() : ltdata);
                showNodata();
            }
        } else {
            //通知列表刷新失败
    ...
    protected List<T> onTaskLoad(boolean isCheckExpired) {
        if (mCacheClazz != null) {
            ...
            if (!isCheckExpired || (System.currentTimeMillis() - date.getTime()) > (mCacheSpan)) {
     ...
     */
    protected OnClickListener mNodataRefreshListener = v -> {
        onRefresh();
        showLoading();
    };
