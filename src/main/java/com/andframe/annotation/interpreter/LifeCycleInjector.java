...
public class LifeCycleInjector {
    ...
    protected static String TAG(Object obj, String tag) {
        if (obj == null) {
            return "LifeCycleInjector." + tag;
        }
        return "LifeCycleInjector(" + obj.getClass().getName() + ")." + tag;
    }
    ...
    public static Object injectLifeCycle(Object handler, Class<? extends Annotation> annotation, Object... params) {
        for (Method method : ReflecterCacher.getMethods(handler)) {
            if (method.isAnnotationPresent(annotation)) {
                try {
                    Object ret = invokeMethod(handler, method, params);
                    if (ret != null) {
                        return ret;
                    }
                } catch (Throwable e) {
                    $.error().handle(e, TAG(handler, "injectOnCreate.invokeMethod.") + method.getName());
                }
            }
        }
        return null;
    ...
    public static void injectOnDispatchFirstResume(Object handler) {
        injectLifeCycle(handler, OnDispatchFirstResume.class);
    }
