...
public class HeaderFooterItemsAdapter<T> extends ItemsViewerAdapterWrapper<T> implements HeaderFooterAdapter<T> {
    ...
    public HeaderFooterItemsAdapter(@NonNull ItemsViewerAdapter<T> wrapped) {
        super(wrapped);
    }
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        try {
            ...
            $.error().handle(e, "AfHeaderFooterAdapter.getView");
    ...
    public void onBindViewHolder(ViewHolderItem<T> holder, int position, List<Object> payloads) {
        try {
            ...
            $.error().handle(e, "AfHeaderFooterAdapter.onBindViewHolder");
        }
    }
    @Override
    ...
    public ViewHolderItem<T> onCreateViewHolder(ViewGroup parent, int viewType) {
        try {
            ...
            if (view == null) {
                ...
                $.error().handle("inflateItem 返回 null", "AfListAdapter.onCreateViewHolder.inflateItem");
            }
            return new ViewHolderItem<>(item, view);
        } catch (Throwable e) {
            $.error().handle(e, "AfListAdapter.onCreateViewHolder");
...
//    }
    ...
    public boolean addHeaderLayout(int layoutId) {
        return addHeader(new ListItemViewer<T>(layoutId) {
            @Override
    ...
    public boolean addHeaderView(@NonNull View view) {
        return addHeader(new ListItemViewer<T>() {
            View mView = view;
            @Override
            public void onBinding(T model, int index) {
            }
    ...
    public boolean addFooterLayout(int layoutId) {
        return addFooter(new ListItemViewer<T>(layoutId) {
            @Override
            public void onBinding(T model, int index) {
            }
        });
    ...
    public boolean addFooterView(@NonNull View view) {
        return addFooter(new ListItemViewer<T>() {
            {mLayout = view;}
            @Override
            public void onBinding(T model, int index) {
            }
