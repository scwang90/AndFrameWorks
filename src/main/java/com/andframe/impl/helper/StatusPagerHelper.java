...
public class StatusPagerHelper<T> extends LoadPagerHelper<T> implements StatusHelper<T> {
    ...
    public boolean checkContentViewStruct(View content) {
        ...
        if (parent == null) {
            $.error().handle("内容视图（ContentView）没有父视图，刷新布局（RefreshLayoutManager/StatusLayoutManager）初始化失败",
                    TAG.TAG(mPager, "AfStatusHelper", "checkContentViewStruct"));
            return false;
        } else if (parent instanceof ViewPager) {
            $.error().handle("内容视图（ContentView）父视图为ViewPager，刷新布局（RefreshLayoutManager/StatusLayoutManager）初始化失败，" +
...
//            mPager.showStatus(StatusLayoutManager.Status.content);
    ...
    public void showInvalidNet() {
        if (mStatusLayoutManager != null) {
            ...
            mPager.toast(AfApp.get().getString(R.string.status_invalid_net));
        }
    }
