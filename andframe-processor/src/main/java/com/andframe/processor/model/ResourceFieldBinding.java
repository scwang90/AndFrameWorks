...
import static com.andframe.processor.constant.ClassNames.BITMAP_FACTORY;
import static com.andframe.processor.constant.ClassNames.CONTEXT_COMPAT;
import static java.util.Collections.singletonList;
import static java.util.Collections.unmodifiableList;

public class ResourceFieldBinding extends ResourceBinding {
    public enum Type {
        BITMAP(new ResourceMethod(BITMAP_FACTORY, "decodeResource", true, 1)),
        BOOL("getBoolean"),
        COLOR(new ResourceMethod(CONTEXT_COMPAT, "getColor", false, 1),
                new ResourceMethod(null, "getColor", false, 23)),
        COLOR_STATE_LIST(new ResourceMethod(CONTEXT_COMPAT, "getColorStateList", false, 1),
                new ResourceMethod(null, "getColorStateList", false, 23)),
        DIMEN_AS_INT("getDimensionPixelSize"),
        DIMEN_AS_FLOAT("getDimension"),
        FLOAT(new ResourceMethod(ANDFRAME_UTILS, "getFloat", false, 1)),
        INT("getInteger"),
    ...
    public ResourceFieldBinding(Id id, String name, Type type) {
        this.id = id;
        this.name = name;
        this.type = type;
    }
